...
(def skate  #{"skate" "skating" "Skate" "Skating"})
(def eat #{"eating" "eat" "restaurants" "food"})
(def visit #{"visit" "visiting" "landmarks" "landmark" "landmarks"})
(def bike #{"bike" "biking" "Bike" "Biking"})
(def sports #{"sports" "sport" "running" "workout"})
(def car #{"car" "parking"})
(def toilet #{"toilet" "restrooms"})           
(defn exit
  "ends dialogue"
  []
...
(defn whichactivity
  "Select the parks where the activity wanted is available"
  []
  (println "What would you like to do in our parks ?")
  (let [x (read-line)]
    (cond )))
